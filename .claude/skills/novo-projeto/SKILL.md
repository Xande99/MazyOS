...
**Se o cliente não se encaixa em nenhum dos nichos catalogados:** não travar o fluxo — registrar no briefing "Nicho de mercado: fora da taxonomia atual do Cérebro" e seguir. Nesse caso a tipografia (ver abaixo) também cai direto no fallback neutro.

**Tipografia por nicho — só roda se o Passo 1.2 registrou identidade visual própria do cliente.** Cor é decisão do cliente (Passo 1.2); tipografia é decisão de brand system já curada por nicho, não se pergunta pro cliente. Não se aplica a quem herda a linha visual da duPolvo — esse projeto segue com Bricolage Grotesque (`dupolvo-theme.css`), sem variação por nicho, igual hoje.

1. Verificar se existe `C:\Users\Xande\Desktop\cerebro\raw\nichos\<nicho>\_tipografia.md`.
   - **Se existir:** extrair o par "Principal" (Heading + Body). Usar o par "Alternativa" no lugar só se a resposta do Passo 1.2 já tiver indicado explicitamente uma preferência de estilo que diverge do racional do par Principal (ex: cliente pediu algo "mais clássico" e o par Principal do nicho é sans-serif moderna, ou vice-versa) — ler a linha "Quando usar" da Alternativa pra confirmar que o caso bate antes de trocar. Guardar o par escolhido pro Passo 3.5.
   - **Se não existir** (nicho catalogado mas sem `_tipografia.md` ainda) **ou se o nicho estiver fora da taxonomia:** usar o fallback neutro Manrope + Inter (mesmo par do nicho "empresa") e avisar no resumo do onboarding que não achou correspondência exata de tipografia pra esse nicho.
2. Registrar o par escolhido (e a fonte da escolha: nicho X / fallback) no `briefing.md`, na mesma seção "Nicho de mercado" — não criar seção nova só pra isso.
...
5. **Se o Passo 1.2 identificou identidade própria do cliente** (não herda a linha da duPolvo) — gerar o `theme.css` do projeto a partir do design system (`_memoria/tokens-contract.md`), em vez de editar `dupolvo-theme.css`:
   ...
   2. Preencher a seção PRIMITIVOS: a partir da cor principal do cliente (hex coletado no Passo 1.2), gerar a escala `--color-brand-50` a `--color-brand-950` em oklch — mesmo hue/chroma da cor original, variando só a luminosidade em 11 degraus (≈0.97 no 50 até ≈0.15 no 950, cor original pousando perto do 500/600). Gerar `--color-neutral-*` como uma escala neutra (chroma baixo, ~0.002–0.01) no mesmo hue de fundo. O próprio placeholder já em `theme.base.css` segue esse padrão de escala — usar como referência de como distribuir os 11 degraus.
   3. Trocar `--font-display`/`--font-body` pelo par de tipografia escolhido no Passo 1.3 (`_tipografia.md` do nicho — Principal, Alternativa, ou fallback Manrope + Inter): `--font-display: "[Heading]", sans-serif;` / `--font-body: "[Body]", sans-serif;`. Nunca redigitar o nome da fonte direto num componente — só aqui, no primitivo.
   4. Instalar as fontes via Fontsource, só os pesos listados em "Pesos recomendados" do `_tipografia.md` — nunca a família toda (comprometeria o Lighthouse Performance ≥90 do pipeline de QA). Preferir o pacote variável (`@fontsource-variable/<slug>`) quando ele existir pro peso necessário — confirmar em fontsource.org antes de instalar, a cobertura muda com o tempo. Se a fonte só tiver pacote estático (ex: Bebas Neue, Anton, Archivo Black, Libre Baskerville — cada peso é uma entrada separada no pacote), importar só os arquivos de peso usados, nunca o index geral da família. Exceção: Clash Display e Satoshi (Fontshare, alternativa do nicho infoprodutos) não têm pacote no Fontsource — self-host manual a partir dos arquivos baixados do Fontshare, e registrar isso como pendência explícita no briefing.
   5. **Tipo A:** atualizar o preload da fonte de display em `src/layouts/Layout.astro` — as duas constantes `preloadFontHref`/`preloadFontType` na frontmatter (comentário no próprio arquivo explica o hook). Default é Bricolage Grotesque (self-hosted, `font/ttf`); trocar pelo import Vite da fonte Fontsource escolhida no item 3 acima, ex: `import preloadFontHref from "@fontsource-variable/<slug>/files/<slug>-latin-wght-normal.woff2?url"; const preloadFontType = "font/woff2";`. **Nunca hardcodar o caminho** — o Fontsource não garante nome de arquivo estável entre versões, o sufixo `?url` resolve pro caminho final com hash do build. Sem isso, sobra um preload apontando pra uma fonte que não é mais usada (`link preloaded but not used` no console) — achado confirmado rodando o starter de verdade (2026-07-21), não só por leitura de código. **Tipo B tem o mesmo padrão fixo em `app/layout.tsx`** (mesmo preload do Bricolage, hardcoded) — ainda não tem o hook equivalente; tratar como pendência separada até alguém aplicar o mesmo fix lá.
   6. Trocar o `@import "./dupolvo-theme.css";` por `@import "./theme.css";` em `src/styles/global.css` (Tipo A) ou `app/globals.css` (Tipo B).
   7. Se algum hex/fonte ainda não estiver definitivo, preencher com o placeholder do `theme.base.css` e marcar como pendência explícita no briefing — nunca inventar cor final.
