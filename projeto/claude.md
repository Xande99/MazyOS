...
- **Dependências**: Dependabot ativo em todos os repos; rodar `npm audit` antes de deploy; desconfiar de pacote novo/pouco usado (typosquatting é real). Versão sempre pinada exata (nunca `^`/`~`) — revisão mensal via `/deps-review` evita a "armadilha do pinning" (nunca receber patch de segurança porque ninguém revisita a versão travada).
- **`ignore-scripts=true` no `.npmrc`** de todo projeto Tipo A/B (raiz e por projeto) — bloqueia scripts `install`/`postinstall`/`preinstall` de qualquer dependência por padrão (proteção contra supply-chain attack via lifecycle script malicioso). Allowlist conhecida e já testada, cujo script de instalação é seguro re-habilitar manualmente se algum dia o build quebrar por binário nativo ausente: `sharp` e `unrs-resolver` (Tipo B), `esbuild` (Tipo A, via toolchain do Astro/Vite). Procedimento: depois de `npm ci`, se o build falhar por binário nativo faltando, rodar `npm rebuild <pacote> --foreground-scripts` só pro pacote específico da allowlist — nunca desligar `ignore-scripts` globalmente pra contornar. Novo pacote com install script legítimo entra na allowlist só depois de auditar o script e documentar aqui.
