...
## Seções específicas deste projeto (não fazem parte da biblioteca cross-nicho)

Criadas pra necessidade pontual do Video e Tal (portfólio de videomaker) — não vieram do `_sintese-geral.md`, então não devem ser copiadas pra outro projeto como se fossem padrão validado; se o caso de uso se repetir em outro cliente, aí sim promover pra biblioteca cross-nicho.

| Componente | Uso |
|---|---|
| `GaleriaProjetos.astro` | Grid de cases com preview em vídeo no hover (desktop) e ícone de play fixo no mobile/touch. Todo o card é um link pra página de case. Ver JSDoc do arquivo pro detalhe de gating por `(hover: hover) and (pointer: fine) and (prefers-reduced-motion: no-preference)`. |
| `LogoWall.astro` | Marcas que o cliente já trabalhou — grayscale por padrão, cor no hover/focus via crossfade de `opacity` (nunca `filter`, conforme a regra de performance do projeto). Grid no desktop, scroll horizontal com snap no mobile. |
