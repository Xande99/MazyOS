...
- **`TierPreco.astro` aceita `precoOriginal` opcional por plano** (adicionado no projeto Cozinha e Tal, 2026-07-22) — renderiza riscado antes do preço final, pro padrão de ancoragem confirmado como robusto em vários nichos (ver `_sintese.md` de cada nicho, "preço ancorado"). Backward-compatible: planos sem essa prop continuam iguais.
- **`TierPreco.astro` aceita `ctaVariante` opcional** (`"brand"` default | `"accent"`, adicionado no projeto Cozinha e Tal, 2026-07-22) — cor do badge/CTA do plano destaque. Default `"brand"` preserva o comportamento original (contrato: `--color-brand` é a cor de CTA por padrão). Só usar `"accent"` quando o projeto tiver registrado essa decisão em `.claude/decisions.md` (caso de cliente com verde=identidade e vermelho=CTA definidos separadamente na própria marca).
