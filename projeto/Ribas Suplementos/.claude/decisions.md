...
## [2026-07-20] Fix: checkout nunca preenchia `cliente_user_id` no insert de `pedidos`

**Decisão:** `criarPedido` (`checkout/actions.ts`) agora chama `supabase.auth.getUser()` (não `getSession()` — valida contra o servidor) logo após criar o client, e inclui `cliente_user_id: user?.id ?? null` no insert de `pedidos`. Convidado (sem sessão) continua gravando `null`, como já era o comportamento esperado — só o caso de cliente logado estava quebrado.
**Motivo:** Achado da auditoria de segurança (Fase 0, item 6): o insert nunca preenchia essa coluna em nenhum caso, então a policy `pedidos: dono vê os próprios` (`cliente_user_id = auth.uid()`) nunca batia — clientes logados nunca viam nada em `/conta`, mesmo tendo feito pedidos de verdade. Bug funcional com efeito de segurança/produto, não uma falha de RLS em si (a policy sempre esteve correta).
**Alternativas descartadas:** Nenhuma — é a correção direta e única do bug, sem trade-off.
**Impacto:** `checkout/actions.ts`. **Não verificado end-to-end** — sem projeto Supabase do Ribas provisionado ainda (mesma pendência de sempre, `ACOES-MANUAIS.md`), não há como testar contra um banco de verdade que o pedido de um cliente logado passa a aparecer em `/conta`. `npm run lint` e `npm run build` passam. Caso 3 de `security/rls-tests.sql` (seção Ribas) já está escrito pra validar isso assim que o projeto existir.
